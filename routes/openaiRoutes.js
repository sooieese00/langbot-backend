...
1. 표현은 원본 문장이 아닌, 격언이나 구문 등을 사전에 나오는 형태로 알려줘.
2. 뜻도 마찬가지로 사전에 나오는 형태로 알려줘.
4. 표현은 ${expressionNumber}개가 정확히 되도록 신경 써.
4. 각 표현의 뜻과 원본 문장, 중요한 포인트, 네가 작성한 새로운 예문과 그 뜻 하나씩 작성해.주요 포인트에서는 관련해서 알아두어야할 문법, 표현이 쓰이는 상황에 대해서 써줘. 원본 문장은 해당 문장 전체를 써줘. 
...
출력할 데이터 형식:(bold처리 없이 이 형식에만 맞춰서 줘)
1. 영어표현
영어표현 뜻: ᄋᄋᄋ  
원본 문장: ᄋᄋᄋ  
원본 문장 해석:ᄋᄋᄋ 
...
router.post('/expressionQuiz', async (req, res) => {
    ...
    try {
        const prompt = `
너는 AI 영어 튜터야. 내가 유튜브 영상을 통해 공부한 영어 표현과 해당 영상의 스크립트를 하단에서 보내줄게. 
이 정보를 바탕으로 내가 공부한 영어 표현을 복습할 수 있는 ${quizNumber}개의 한국어 표현과 영어 답을 만들어줘. 
공부한 것을 바탕으로 각 퀴즈를 만들도록 꼭 신경써줘. 
형식은 아래에 제시해줄게. 
...
문제에 대해서 의미상, 문법상 영어 답이 정확하게 맞으면 스크립트 상의 표현에 상관없이 정답여부에 정답입니다라고 출력해주고, 대신 피드백을 더욱 자세하게 재공해줘. 
다만, 스크립트 상의 문맥과 맞지 않으면 오답처리를 해주고 그 이유를 함께 피드백으로 제공해줘.
한국말로 쓴 경우는 무조건 오답처리 해줘. 
