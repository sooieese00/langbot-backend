...
실생활에서 자주 사용하는 표현들 중에서, 난이도 ${expressionLevel}에 해당하는 표현 ${expressionNumber}개를 아래 조건에 맞춰 추출해줘.
...
router.post('/expressionQuiz', async (req, res) => {
    ...
    try {
        const prompt = `
        너는 AI 영어 튜터야. 내가 유튜브 영상을 통해 공부한 영어 표현과 해당 영상의 스크립트를 하단에 보내줄게. 
        이 정보를 바탕으로 내가 공부한 영어 표현을 복습할 수 있는 ${quizNumber}개의 한국어 표현과 영어 답을 만들어줘. 
        표현이 사용된 맥락과 유사한 상황을 반영해서 퀴즈를 구성해줘. 또한, 각 퀴즈의 난이도는 학습한 영어 표현의 난이도와 일치하도록 신경 써줘.
        
        형식은 아래와 같아:
        출력할 데이터 형식 (bold처리 없이 이 형식에 맞춰서 줘):
        1. 한국어 표현 : ᄋᄋᄋ
        영어 답 : ᄋᄋᄋ
...
router.post('/feedback', async (req, res) => {
    ...
    try {
        ...
        출력할 데이터 형식 (bold 처리 없이 이 형식에 맞춰서 출력해줘):
        정답여부: 정답입니다 / 오답입니다
        피드백: ㅇㅇㅇ
        유사표현 제안: ㅇㅇㅇ / ㅇㅇㅇ / ㅇㅇㅇ
        모범답안: ㅇㅇㅇ
